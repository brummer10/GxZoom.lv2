...
#define GTK_ZOOM_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_ZOOM_KNOB, GtkZoomKnobPrivate))

G_DEFINE_TYPE(GtkZoomKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkZoomKnobPrivate GtkZoomKnobPrivate;

struct _GtkZoomKnobPrivate
{
    int knob_x;
    int knob_y;
    int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
    /** check resize **/
    int grow;
    GtkZoomKnobPrivate *priv = GTK_ZOOM_KNOB_GET_PRIVATE(widget);
    ...
    if (priv->model != 3) {
        ...
        GtkZoomKnob *knob = GTK_ZOOM_KNOB(widget);
...
{
    g_assert(GTK_IS_ZOOM_KNOB(widget));
    GtkZoomKnob *knob = GTK_ZOOM_KNOB(widget);
    GtkZoomKnobPrivate *priv = GTK_ZOOM_KNOB_GET_PRIVATE(knob);
...
{
    GtkZoomKnob *knob = GTK_ZOOM_KNOB(object);
...
/****************************************************************
 ** init the GtkZoomKnobClass
 */

static void gtk_knob_class_init (GtkZoomKnobClass *klass)
{
...
//--------- connect the events with funktions
    ...
    g_type_class_add_private(obj_class, sizeof (GtkZoomKnobPrivate));
}
...
static void gtk_knob_init (GtkZoomKnob *knob)
{
...
{
    GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_ZOOM_KNOB, NULL ));
